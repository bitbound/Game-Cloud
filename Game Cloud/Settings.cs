...
namespace Game_Cloud
{
    public class Settings
    {
        ...
        public void Save()
        {
            if (SettingsTemp.Current.Uninstall)
            {
                return;
            }
            var strPath = SettingsTemp.AppDataFolder;
        ...
        public void LoadGlobal()
        {
            ...
            {
#if DEBUG
                var globalSettings = JsonHelper.Decode<Settings>(File.ReadAllText(strPath + @"\Settings - Global.json"));
#else
                var filePath = strPath + @"\Settings - Global.json";
                var globalSettings = JsonHelper.Decode<Settings>(File.ReadAllText(filePath));
#endif
...
#else
                var filePath = strPath + @"\Settings - " + Settings.Current.AccountInfo.AccountName + ".json";
